...
class InstaDM(object):
    ...
    def __init__(self, username, password, headless=True, instapy_workspace=None, profileDir=None):
        self.selectors = {
            "accept_cookies": "(//button[normalize-space()='Allow all cookies'])[1]",
            "home_to_login_button": "(//button[@type='submit'])[1]",
        ...
        chrome_options.add_argument("executable_path=your/chromedriver/path")
    ...
    def login(self, username, password):
        ...
        self.__wait_for_element__(self.selectors["accept_cookies"], 'xpath', 10)
        accept_cookies = self.__get_element__(self.selectors["accept_cookies"], 'xpath')
        if accept_cookies is not None:
            accept_cookies.click()
            self.__random_sleep__(3, 5)
        else:
            print("Login Failed: Accept cookies button not found")
        login_button = self.__get_element__(
            self.selectors["home_to_login_button"], 'xpath')
        self.__random_sleep__(3, 5)
        ...
        if not self.__wait_for_element__(self.selectors['username_field'], 'name', 10):
            ...
            'name', self.selectors['username_field']).send_keys(username)
            self.driver.find_element(
            'name', self.selectors['password_field']).send_keys(password)
